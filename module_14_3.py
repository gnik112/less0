...
async def out_product(message, nom, info):
    await message.answer(f"Название: Питание{nom} | Описание: {info} | Цена: {nom * 100}")
    with open(f'files/0{nom}.webp', "rb") as pic:
        await message.answer_photo(pic)


@dp.message_handler(text=["Купить"])
async def get_buying_list(message):
    await out_product(message, 1, "Хлебцы Деликат")
    await out_product(message, 2, "Хлебцы Самарские")
    await out_product(message, 3, "Хлебцы ZAI CRACKER")
    await out_product(message, 4, "Сок из квашеного корня сельдерея")
